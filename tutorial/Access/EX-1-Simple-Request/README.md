...
like set headers, or send a request body. If we look up the `/auth/accesstoken` request ([here](https://api.tradovate.com/#operation/accessTokenRequest)) on the official docs, we can see that we do need to send some data.
